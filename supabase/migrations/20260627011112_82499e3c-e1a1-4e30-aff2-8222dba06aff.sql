ALTER TABLE public.op_dispatches ADD COLUMN IF NOT EXISTS finished_at timestamptz;

UPDATE public.op_dispatches SET finished_at = coalesce(updated_at, created_at)
WHERE status = 'finalizado' AND finished_at IS NULL;
